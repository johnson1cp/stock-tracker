...
export function SectorHeatMap() {
  ...
  const getColor = (percentChange) => {
    if (percentChange === 0 || isNaN(percentChange)) return 'var(--card-bg)';

    const intensity = Math.min(Math.abs(percentChange) / 5, 1);

    if (percentChange > 0) {
      const r = Math.round(20 + (30 - 20) * (1 - intensity));
      const g = Math.round(80 + (200 - 80) * intensity);
      const b = Math.round(50 + (80 - 50) * (1 - intensity));
      return `rgb(${r}, ${g}, ${b})`;
    } else {
      const r = Math.round(80 + (220 - 80) * intensity);
      const g = Math.round(50 + (50 - 50) * (1 - intensity));
      const b = Math.round(50 + (70 - 50) * (1 - intensity));
      return `rgb(${r}, ${g}, ${b})`;
    }
  };

  const getTextColor = (percentChange) => {
    const intensity = Math.min(Math.abs(percentChange) / 3, 1);
    return intensity > 0.3 ? '#fff' : 'var(--text-primary)';
  };
            ...
            <div className="sector-tiles">
              {(sectorData[sector.id] || []).map((stock, idx) => (
                <div
                  ...
                  style={{
                    backgroundColor: getColor(stock.dp),
