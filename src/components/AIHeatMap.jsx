...
const SHEET_URL = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vQIJCt2JRZmPht5pwAP8HW9YWWYcFcG8II9KJ32YWm36B7plT2nriGPFQlDlWnD0z9Dp7lPqjVDwxS6/pub?gid=863794709&single=true&output=csv';

const TIME_PERIODS = [
  { key: '1D', label: '1D', colIndex: null },  // Uses changepct
  { key: '1W', label: '1W', colIndex: 27 },    // Header "7" - price 7 days ago
  { key: '1M', label: '1M', colIndex: 28 },    // Header "30" - price 30 days ago
  { key: '3M', label: '3M', colIndex: 29 },    // Header "90" - price 90 days ago
  { key: '6M', label: '6M', colIndex: 30 },    // Header "182" - price 182 days ago
  { key: 'YTD', label: 'YTD', colIndex: 31 },  // Header "22" - price at year start
  { key: '1Y', label: '1Y', colIndex: 32 },    // Header "365" - price 365 days ago
  { key: '3Y', label: '3Y', colIndex: 33 },    // Header "1095" - price 1095 days ago
  { key: '5Y', label: '5Y', colIndex: 34 },    // Header "1825" - price 1825 days ago
  { key: '10Y', label: '10Y', colIndex: 35 },  // Header "3650" - price 3650 days ago
];
...
const COMPANY_COL_INDEX = 0;     // Company
const SECTOR_COL_INDEX = 2;      // Sector
const CATEGORY_COL_INDEX = 5;    // Category (used as industry)
const VOLUME_COL_INDEX = 9;      // volume
const MARKETCAP_COL_INDEX = 11;  // marketcap
const RELVOL_COL_INDEX = 12;     // relvol
const TRADETIME_COL_INDEX = 13;  // tradetime
...
export function AIHeatMap() {
  ...
  const fetchStocks = useCallback(async () => {
    try {
      ...
      const priceIdx = headers.findIndex(h => h.trim() === 'price');
      const pctChangeIdx = headers.findIndex(h => h.trim() === 'changepct');
      const changeIdx = headers.findIndex(h => h.trim() === 'change');

      // Get reference trade time from row 2 to calculate intraday factor
      ...
      for (let i = 1; i <= 180 && i < lines.length; i++) {
        ...
        // Parse time period changes - columns contain historical prices, calculate % change
        const periodChanges = {};
        TIME_PERIODS.forEach(period => {
          if (period.colIndex !== null) {
            const histPrice = parseFloat(cols[period.colIndex]);
            if (histPrice && !isNaN(histPrice) && histPrice > 0) {
              periodChanges[period.key] = ((price - histPrice) / histPrice) * 100;
            } else {
              periodChanges[period.key] = 0;
            }
          }
        });
        periodChanges['1D'] = pctChange;

        // Parse market cap, company, sector, category, volume
        ...
        const industry = cols[CATEGORY_COL_INDEX]?.trim() || '';
      ...
      console.error('Failed to fetch AI heatmap data:', err);
      setLoading(false);
    }
  }, []);
  ...
  return (
    <div className="heatmap-container ai-heatmap">
